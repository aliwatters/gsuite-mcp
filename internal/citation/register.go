...
func RegisterTools(s *server.MCPServer) {
	s.AddTool(mcp.NewTool("citation_create_index",
		mcp.WithDescription("Create a citation index Sheet in a Drive folder for document tracing."),
	...
	s.AddTool(mcp.NewTool("citation_add_documents",
		mcp.WithDescription("Extract and chunk documents into a citation index."),
	...
	s.AddTool(mcp.NewTool("citation_save_concepts",
		mcp.WithDescription("Save extracted concepts for chunks (from LLM analysis)."),
		mcp.WithString("index_id", mcp.Required(), mcp.Description("Index name")),
	...
	s.AddTool(mcp.NewTool("citation_save_summary",
		mcp.WithDescription("Save a hierarchical summary (level 0=chunk, 1=doc, 2=corpus)."),
		mcp.WithString("index_id", mcp.Required(), mcp.Description("Index name")),
	...
	s.AddTool(mcp.NewTool("citation_list_indexes",
		mcp.WithDescription("List configured citation indexes."),
		common.WithAccountParam(),
	), HandleCitationListIndexes)

	s.AddTool(mcp.NewTool("citation_get_overview",
		mcp.WithDescription("Get index summary: doc count, concepts, corpus summary."),
		mcp.WithString("index_id", mcp.Required(), mcp.Description("Index name")),
		common.WithAccountParam(),
	), HandleCitationGetOverview)

	s.AddTool(mcp.NewTool("citation_lookup",
		mcp.WithDescription("Search chunks by keyword (FTS5). Returns snippets, not full text."),
		mcp.WithString("index_id", mcp.Required(), mcp.Description("Index name")),
	...
	s.AddTool(mcp.NewTool("citation_get_chunks",
		mcp.WithDescription("Get full chunk text + location by IDs."),
		mcp.WithString("index_id", mcp.Required(), mcp.Description("Index name")),
	...
	s.AddTool(mcp.NewTool("citation_verify_claim",
		mcp.WithDescription("Find chunks supporting a claim. Returns candidates with citations."),
		mcp.WithString("index_id", mcp.Required(), mcp.Description("Index name")),
	...
	s.AddTool(mcp.NewTool("citation_format_citation",
		mcp.WithDescription("Format a chunk as a human-readable citation string."),
		mcp.WithString("index_id", mcp.Required(), mcp.Description("Index name")),
	...
	s.AddTool(mcp.NewTool("citation_refresh",
		mcp.WithDescription("Refresh index: detect updated/removed/renamed files and re-index."),
		mcp.WithString("index_id", mcp.Required(), mcp.Description("Index name")),
		common.WithAccountParam(),
